...
sudo docker run --volumes-from $OVPN_DATA --restart=always --name $OVPN_CONTAINER_NAME -d -p $HOST_PORT:$HOST_PORT/$TRANSPORT_PROTOCOL --cap-add=NET_ADMIN kylemanna/openvpn
